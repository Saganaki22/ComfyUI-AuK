...
Input must be a cappella — clean solo singing with no instrumental backing. If the track has accompaniment, extract singing first, then connect that output to the second Instruction Encode.audio for the lyric edit. Use lyrics that occur in the source and aim for similar syllable counts. Set seconds=0 to retain the melody timing. The workflow does not automatically remix the edited vocal with the accompaniment.
