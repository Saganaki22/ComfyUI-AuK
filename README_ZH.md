...
下表使用十进制 GB（1 GB = 1,000,000,000 字节），表示文件大小，不是显存需求。全部十个检查点文件均已上传，下载链接已核对。
...
在 FP32 与 BF16 检查点上经过反复测试，我的结论是：遗憾的是，该模型的指令遵循度并不强。声音克隆、TTS 与部分编辑任务可用，但许多任务并不总是有效。按我的测试从最可靠到最不可靠排序：
